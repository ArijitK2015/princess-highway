...
Checkout.prototype = {
    ...
    _onSectionClick: function(event) {
        var section = $(Event.element(event).up().up().up());
    ...
    reloadStep: function(prevStep) {
        var updater = new Ajax.Request(this.progressUrl, {
            ...
            onSuccess: function(transport){
                ...
                $(prevStep + '-progress-opcheckout').update(response);
                $(prevStep + '-progress-opcheckout-offcanvas').update(response);
            },
    ...
    gotoSection: function (section, reloadProgressBlock) {
        ...
        jQuery("#checkoutSteps .in").collapse("toggle");
        // Open new section
        jQuery("#checkout-step-"+section).collapse("toggle");
        if(!reloadProgressBlock) {
    ...
    resetPreviousSteps: function () {
        ...
        for (var i = stepIndex; i < this.steps.length; i++) {
            ...
            var progressDiv = nextStep + '-progress-opcheckout-offcanvas';
            if ($(progressDiv)) {
                //Remove the link
                $(progressDiv).select('.changelink').each(function (item) {
                    item.remove();
