...
function addRow(tableID) {
    ...
    element2.type = "text";
    element2.className = "form-control input-text validate-email";
    cell2.appendChild(element2);
}

function addEmailField(divId, fa) {
    var count = jQuery('#'+divId+' > .email').length;
    if (fa)
    {
        buttonText = '<i class="fa fa-times"></i>';
    }
    else buttonText = 'Remove';
    jQuery('#'+divId).append('<div class="email form-group"><div class="input-group"><input type="textbox" name="friend'+(count+1)+'" id="friend'+(count+1)+'" class="form-control input-text validate-email" value="" placeholder="Your Friend\'s Email"><span class="input-group-btn"><button onclick="removeEmailField(\'friend'+(count+1)+'\')" class="btn btn-default">'+buttonText+'</button></span></div></div>');
}
