...
class MDN_EasyReview_FrontController extends Mage_Core_Controller_Front_Action {
    ...
    public function PostReviewsAction() {
        try {
            ...
            $helper = Mage::helper('EasyReview/Notifier');
            $order = $helper->getOrder($security_key);
            Mage::app()->getLocale()->emulate($order->getStoreId());

            //log in customer
            $customerId = $order->getcustomer_id();
            Mage::helper('EasyReview/Login')->logCustomer($customerId);

            //display form
            Mage::register('easyreview_current_order', $order);
            $this->loadLayout();
            $this->renderLayout();
        } catch (Exception $ex) {
