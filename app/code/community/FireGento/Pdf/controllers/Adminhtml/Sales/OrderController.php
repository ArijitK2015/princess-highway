    ...
    extends Mage_Adminhtml_Sales_OrderController
{
    ...
    TODO
    */
    public function pdforderssAction()
    {
    }    
    ...
    public function printAction()
    {
        ...
        if ($orderId = $this->getRequest()->getParam('order_id')) {
            if ($order = Mage::getModel('sales/order')->load($orderId)) {
                //Mage::helper('firegento_pdf')->log(sprintf("%s->order=%s", __METHOD__, get_class($order)));
                
                //$pdf = Mage::getModel('sales/order_pdf_order')->getPdf(array($order));
                //$pdf = Mage::getModel('firegento_pdf/order_pdf_order')->getPdf(array($order));
                $pdf = Mage::getModel('firegento_pdf/order')->getPdf(array($order));
                //Mage::helper('firegento_pdf')->log(sprintf("%s->pdf=%s", __METHOD__, get_class($pdf)));
                
                $this->_prepareDownloadResponse(
                    $filename = Mage::helper('firegento_pdf')->getExportFilename('order', $order),
                    $pdf->render(),
                    'application/pdf'
                );
                //Mage::helper('firegento_pdf')->log(sprintf("%s->filename: %s", __METHOD__, $filename));
            }
        }
        else {
