    ...
    public function draw() {
        ...
        if ($options) {
            foreach ($options as $option) {
                ...
                $text = sprintf("%s: %s", strip_tags($option['label']), $_printVal);
                $lines[][] = array(
                    'text' => Mage::helper('core/string')->str_split($text, 40, false, true),
                    'font' => 'bold',
                    'feed' => $pdf->margin['left'] + 130 + 10
                );
