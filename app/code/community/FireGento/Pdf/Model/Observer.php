...
class FireGento_Pdf_Model_Observer
{
    ...
    public function addInvoiceNotes(Varien_Event_Observer $observer)
    {
        ...
        $this->_addInvoiceTaxMessage($observer);
        return $this;
    }
     ...
     * @return FireGento_Pdf_Model_Observer
     */
    private function _addInvoiceTaxMessage(Varien_Event_Observer $observer) {
        
        if (!Mage::helper('firegento_pdf')->showTaxMessage($observer->getOrder()->getBillingAddress()->getCountryId())) {
            return $this;
        }
                
        $result = $observer->getResult();
        $notes = $result->getNotes();
        $message = Mage::helper('firegento_pdf')->getTaxMessage();
        if (empty($message)) {
            return $this;
        }
        $notes[] = $message;
    ...
    */
    public function adminhtmlWidgetContainerHtmlBefore(Varien_Event_Observer $observer) {
        ...
        if(
            ...
            $printUrl = $block->getUrl('*/*/print', array(
                'order_id' => $order->getId()
            ));
            //Mage::helper('firegento_pdf')->log(sprintf("%s->printUrl=%s", __METHOD__, $printUrl));
