<?php
/**
 * Class FactoryX_CustomReports_Block_Nocrosssells_Grid
 *
 */

class FactoryX_CustomReports_Block_Nocrosssells_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    {
        parent::__construct();
        $this->setId('nocrosssellsReportGrid');
    ...
    {
        $product_with_crosssell_id = $args['row']['product_id'];

        if (!in_array($product_with_crosssell_id, $this->filterArray))
        {
            array_push($this->filterArray, $product_with_crosssell_id);
        }
    }
     ...
     * @return array
     */
    public function getFilterCrosssellsItems()
    {
        $crosssellsItemsCollection = Mage::getModel('catalog/product_link')->useCrossSellLinks()
            ->getLinkCollection()
            ->addFieldToSelect('product_id');

        // Call iterator walk method with collection query string and callback method as parameters
        // Has to be used to handle massive collection instead of foreach
        Mage::getSingleton('core/resource_iterator')->walk($crosssellsItemsCollection->getSelect(), array(array($this, 'fillArray')));
    ...
    protected function _prepareCollection()
    {
        ...
        $collection = Mage::getResourceModel('catalog/product_collection')
                        ...
                        ->addAttributeToFilter('entity_id',array('nin'=>$this->getFilterCrosssellsItems()));
        if (!empty($this->filterArray)) {
            $collection->addAttributeToFilter('entity_id',array('nin'=>$this->getFilterCrosssellsItems()));
        }
        */

        $collection = Mage::getResourceModel('reports/product_collection');
        // set type so the correct getSelectCountSql() logic is used
        $collection->setSelectCountSqlType(FactoryX_CustomReports_Model_Reports_Resource_Product_Collection::SELECT_COUNT_SQL_TYPE_CUSTOM_REPORT_CROSSSELLS);
        ...
        $collection->getSelect()
            ...
            ->joinLeft(
                ...
                'e.entity_id = cpl.product_id AND cpl.link_type_id = 5',
                array('count(cpl.linked_product_id) as crosssell_count')
            )
            ->joinInner(
            ...
            ->group('e.sku')
            ->having(sprintf("count(linked_product_id) < %d", $this->_getMinCrosssellsCount()));

        $session = Mage::getSingleton('core/session');
        $sortOrder = 'name ASC';
        if ($session->getNocrosssellsSort() && $session->getNocrosssellsDir()) {
            //Mage::helper('customreports')->log(sprintf("session: %s\n", print_r($session->getData(), true)));
            $sortOrder = sprintf("%s %s", $session->getNocrosssellsSort(), strtoupper($session->getNocrosssellsDir()));
        }
     ...
     */
    private function _getMinCrosssellsCount() {
        $minUpsellsCount = FactoryX_CustomReports_Block_Nocrosssells::DEFAULT_MIN_CROSSSELLS;
        $session = Mage::getSingleton('core/session');
        if ($session->getMinNocrosssellsCount()) {
            ...
            if (is_numeric($session->getMinNocrosssellsCount())) {
                //Mage::helper('customreports')->log(sprintf("%s->is_numeric:%s", __METHOD__, $session->getMinNocrosssellsCount()) );
                $minUpsellsCount = $session->getMinNocrosssellsCount();
            }
        }
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('crosssell_count', array(
            'header'        => Mage::helper('reports')->__('Crosssells'),
            'width'         => '50px',
            'index'         => 'crosssell_count',
            //'filter'    => false,
            //'sortable'  => false
        ));
            ...
            'actions'       => array(
                array(
                    ...
                    'url'       => array(
                        ...
                        'params'    => array('active_tab' => 'crosssell')
                    ),
                    'target'    => '_blank',
        ...
        $this->addExportType('*/*/exportNocrosssellsCsv', Mage::helper('reports')->__('CSV'));
        $this->addExportType('*/*/exportNocrosssellsExcel', Mage::helper('reports')->__('Excel'));
