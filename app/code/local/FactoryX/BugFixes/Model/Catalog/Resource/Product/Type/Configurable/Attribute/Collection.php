 ...
 */
class FactoryX_BugFixes_Model_Catalog_Resource_Product_Type_Configurable_Attribute_Collection 
    ...
    {
        // apparently 1.9.1.x is also effected, but uses a different fix
        ...
        if ($this->count()) {
            ...
            foreach ($this->_items as $item) {
                ...
                foreach (array_intersect_key($optionsByValue, array_flip($toAdd)) as $optionValueKey => $optionValue) {
                    ...
                    $key = sprintf("%s:%s", $item->getId(), $optionValueKey);
                    if (!isset($values[$key])) {
