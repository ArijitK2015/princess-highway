<?php
/**
 *	Fix _updateDataWithCategoryColumns and export functions
 */
class FactoryX_BugFixes_Model_ImportExport_Export_Entity_Product extends Mage_ImportExport_Model_Export_Entity_Product
{
    /**
    ...
    protected function _updateDataWithCategoryColumns(&$dataRow, &$rowCategories, $productId)
    {
		...
		$cat = self::COL_ROOT_CATEGORY;
		// categoryId not in _rootCategories has been causing an error
		if (array_key_exists($categoryId, $this->_rootCategories) && isset($this->_rootCategories[$categoryId])) {
			$dataRow[$cat] = $this->_rootCategories[$categoryId];
		}		
    ...
    public function export()
    {
        ...
        while (true) {
            ...
            $cnt = 0;
            $limit = 10;
            while (($cnt++ < $limit) && $product = $configurableProductsCollection->fetchItem()) {
            	// Mage_Catalog_Model_Product_Type_Configurable
            	$instance = $product->getTypeInstance(true);
            	if (method_exists($instance, "getConfigurableOptions")) {
	                $productAttributesOptions = $instance->getConfigurableOptions($product);
	            }
                else {
                	$attrs = $instance->getUsedProductAttributes($product);
                	$productAttributesOptions = Mage::getResourceSingleton('catalog/product_type_configurable')->getConfigurableOptions($product, $attrs);
                }
