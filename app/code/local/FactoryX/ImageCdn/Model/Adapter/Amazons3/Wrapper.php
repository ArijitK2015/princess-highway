...
/**
 * Class FactoryX_ImageCdn_Model_Adapter_Amazons3_Wrapper
 */
class FactoryX_ImageCdn_Model_Adapter_Amazons3_Wrapper
{
    private $key;
    ...
    public function uploadFile($bucket_name, $s3_path, $fs_path, $web_accessible = false, $headers = null)
    {
        ...
        if (Mage::getStoreConfig('imagecdn/general/proxy')){
            $curl_opts['CURLOPT_PROXY'] = Mage::getStoreConfig('imagecdn/general/proxy');
        }
        if(is_null($headers))
            $headers = array();
     ...
     * @param $bucket_name
     * @param $s3_path
     * @param int $seconds_till_expires
     * @return string
     */
    ...
    private function sendRequest($request, $headers = null, $curl_opts = null)
    {
        ...
        if (Mage::getStoreConfig('imagecdn/general/proxy')){
            curl_setopt($ch, CURLOPT_PROXY, Mage::getStoreConfig('imagecdn/general/proxy'));
        }
