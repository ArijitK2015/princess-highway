 ...
 * CDN adapter for FTP
 */
class FactoryX_ImageCdn_Model_Adapter_Ftp extends FactoryX_ImageCdn_Model_Adapter_Abstract
{
	/**
	 * Connection handle
	 */
	 ...
	 *
	 * @return FactoryX_ImageCdn_Model_Varien_Ftp
	 */
	private function auth() { 
		if(is_null($this->conn)) {
	    	...
	    	$this->conn->open(array('host'=>$ftp_host,'user'=>$ftp_user,'password'=>$ftp_pass,'port'=>$ftp_port));
		}
		return $this->conn;
	}
	...
	protected function _onConfigChange() {
		if(!function_exists('ftp_connect')) {
			...
			$session->addWarning('This server does not support FTP. You must change your settings or enable the FTP modules on your server. See the <a href="http://us.php.net/manual/en/ftp.installation.php" target="_blank" rel="noopener noreferrer">PHP manual</a> for more infomation.');
			return false;		
		}
