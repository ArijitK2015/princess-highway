...
/**
 * Class FactoryX_Abandonedcarts_Model_Notifier
 */
class FactoryX_Abandonedcarts_Model_Notifier extends Mage_Core_Model_Abstract
{
	...
	public function generateRecipients($args)
	{
		...
		if (!array_key_exists($args['row']['customer_email'], $this->_recipients)) {
			...
			$emailTemplateVariables['link']	= (Mage::helper('abandonedcarts')->isShortenerEnabled() ? Mage::helper('abandonedcarts')->shortenUrl($this->_generateUrl($token, $this->_currentStoreId)) : $this->_generateUrl($token, $this->_currentStoreId));
		} else {
		...
		foreach ($this->_getSaleRecipients() as $email => $recipient) {
			...
			Mage::getModel('abandonedcarts/log')->setData(
				array(
					...
					'type'				=>	FactoryX_Abandonedcarts_Model_Log::TYPE_SALES,
					'comment'			=>	$comment,
	...
	protected function _sendEmails($dryrun, $testemail)
	{
		...
		foreach ($this->_getRecipients() as $email => $recipient) {
			...
			Mage::getModel('abandonedcarts/log')->setData(
				array(
					'customer_email'	=>	$email,
					'type'				=>	FactoryX_Abandonedcarts_Model_Log::TYPE_NORMAL,
					'comment'			=>	$comment,
					'store'				=>	$recipient['store_id'],
					'dryrun'			=>	$dryrun ? 1 : 0
				)
