...
class FactoryX_Framework_Model_Observer
{
     ...
     */
    public function modifyOutput(Varien_Event_Observer $observer)
    {
        // Get the rendered block
        $block = $observer->getBlock();

        $newHtml = $this->addBootstrapClasses($observer, $block);
     ...
     * @return mixed
     */
    protected function addBootstrapClasses(Varien_Event_Observer $observer, $block)
    {
        if ($block instanceof Fontis_Australia_Block_Autocomplete) {
            $newHtml = $this->_addUnstyledLink($observer->getTransport()->getHtml());
            return $newHtml;
        } else {
            if ($block instanceof Mage_Wishlist_Block_Customer_Sidebar) {
                $newHtml = $this->_addBadge(array('<small>(', ')</small>'), array('<span class="badge">', '</span>'),
                ...
                if ($block instanceof Mage_Wishlist_Block_Links) {
                    ...
                    if (Mage::helper('framework')->isWishlistBadge()) {
                        $newHtml = $this->_addBadge(array('>My Wishlist (', ' item)<'),
                            array(">My Wishlist <span class='badge'>", '</span><'), $newHtml);
                        $newHtml = $this->_addBadge(array('>My Wishlist (', ' items)<'),
                            array(">My Wishlist <span class='badge'>", '</span><'), $newHtml);
                    } else {
                    ...
                    if ($block instanceof MageWorx_XSitemap_Block_Page_Template_Links
                        ...
                        if (Mage::helper('framework')->isCartBadge()) {
                            $newHtml = $this->_addBadge(array('>My Cart (', ' item)<'),
                                array(">My Cart <span class='badge'>", '</span><'), $newHtml);
                            $newHtml = $this->_addBadge(array('>My Cart (', ' items)<'),
                                array(">My Cart <span class='badge'>", '</span><'), $newHtml);
                        } else {
                            $newHtml = str_replace(' item)', ')', $newHtml);
                            $newHtml = str_replace(' items)', ')', $newHtml);
                        ...
                        if ($block instanceof Mage_CatalogSearch_Block_Autocomplete) {
                            $newHtml = $this->_addUnstyledLink($observer->getTransport()->getHtml());
