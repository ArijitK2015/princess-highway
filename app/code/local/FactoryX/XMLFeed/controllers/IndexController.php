...
class FactoryX_XMLFeed_IndexController extends Mage_Core_Controller_Front_Action {
	...
	<product type="%PROD_TYPE%">
		...
		<brand>Alannah Hill</brand>
		%PROD_VARS%
	</product>';

	public function indexAction() {
		//Mage::helper('xmlfeed')->log(sprintf("%s", __METHOD__));
			...
			if (count($images)) {
				...
				foreach($images as $image) {
					...
					if (count($label) > 1) {
						...
						if (preg_match("/(front|back|swatch|detail)/", $pos)) {
							$aImages["$col"]["$pos"] = $url;
							$noLabel = false;
						}
					...
					if ($noLabel && $i < count($labels)) {
					    Mage::helper('xmlfeed')->log(sprintf("Error: %s->no label for product [%s] image: %s", __METHOD__, $product->getSku(), $url));
