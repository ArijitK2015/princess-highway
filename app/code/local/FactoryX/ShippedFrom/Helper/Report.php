...
/**
 * Class FactoryX_ShippedFrom_Helper_Report
 */
class FactoryX_ShippedFrom_Helper_Report extends Mage_Core_Helper_Abstract
{
    ...
    public function generateReport($range)
    {
        ...
        $this->hlp()->log(sprintf("SQL: %s", $sql));
        if (count($_shipmentCollection) == 0) {
            Mage::throwException(
                Mage::helper('shippedfrom')->__("No shipments found between [%s - %s]", date("Y-m-d H:i:s", $range['from']), date("Y-m-d H:i:s", $range['to']))
            );
        }
        ...
        foreach ($_shipmentCollection as $s) {
            ...
            */
            ...
            /*
            $this->hlp()->log(sprintf("%s->shippedItemIds[%s|%s]: %s",
                __METHOD__,
        ...
        /*
        $this->hlp()->log(sprintf("%s",
        print_r($shippedFromStores, true))
        );
        */
        $report = Mage::helper('shippedfrom/report')->formatReport($shippedFromStores);

        $this->hlp()->log(sprintf("****"));
        $this->hlp()->log(sprintf("%s", $report));
        $this->hlp()->log(sprintf("****"));

        $subject = sprintf(
            "%s - Shipped From Stores Report [%s - %s]",
    ...
    {
        if ($recipientEmail) {
            $this->hlp()->log(sprintf("%s->recipientEmail=%s", __METHOD__, $recipientEmail));
        }
        ...
        if ($additionalEmails) {
            ...
            foreach ($addr as $recipientEmail) {
                if (filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
                    $this->hlp()->log(sprintf("email report to '%s'", $recipientEmail));
                    ...
                    $this->hlp()->log(sprintf("%s->additional email '%s' invalid!", __METHOD__, $recipientEmail));
                }
            }
        }
     ...
     * @return array
     */
    public function getDateRange($shippingDate, $weekDaysToSendOn = array(), $inclusive = false)
    {
        if ($shippingDate) {
            ...
            $this->hlp()->log(sprintf("hourOfTheDay=%s", $hourOfTheDay));
    ...
    public function getTestShipmentDate()
    {
        ...
        $this->hlp()->log(sprintf("%s->testDate: %s", __METHOD__, $testDate));
        return $testDate;
    }
