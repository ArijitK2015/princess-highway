...
class Googl
{
     ...
     * @param null $proxy
     */
    function __construct($apiKey = null, $proxy = null) {
    ...
    public function shorten($url, $extended = false) {
        ...
        // CURLOPT_RETURNTRANSFER = true, curl_exec will return result on success, FALSE on failure
        $retVal = curl_exec($this->ch);
        if ($retVal) {
            if ( $extended || $this->extended) {
                return json_decode($retVal);
            } else {
                $ret = json_decode($retVal)->id;
                self::$buffer[$url] = $ret;
                return $ret;
            }
        }
        else {
            throw new Exception(sprintf("failed to request %s, error: %s", $this->target, curl_error($this->ch)) );
        }
    }
