...
class FactoryX_Contests_Block_Adminhtml_Contests_Edit_Tab_Media extends Mage_Adminhtml_Block_Widget_Form
{
	...
	protected function _prepareForm() 
	{
        ...
        $fieldset->addField('image_url', 'contestimage', array(
			...
			'required'  => true,
			'name'      => 'image_url',
			'note'      => Mage::helper('contests')->__('Recommended image size: 960px wide x 270px high for a Refer A Friend contest (<a target="_blank" href="%s">example</a>) or 354px wide x 658px high for a Give Away contest (<a target="_blank" href="%s">example</a>).', $this->_exampleRafPictureUrl, $this->_exampleGaPictureUrl),
		));
        ...
        $fieldset->addField('email_image_url', 'contestimage', array(
			...
			'note'      => Mage::helper('contests')->__('Only for refer a friend contest, recommended image size: 980px wide and any height. (<a target="_blank" href="%s">example</a>)',$this->_exampleEmailPictureUrl),
		));
