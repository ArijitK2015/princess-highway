<?php
/**
 * Class FactorYX_Gua_Model_Observer
 */
class FactorYX_Gua_Model_Observer
{
	...
	public function customerLogin(Varien_Event_Observer $observer)
	{
		...
		// Delete the session variable just in case
		Mage::getModel('core/session')->unsGuaCheckout();
		// Add the cart products to the session variable
		foreach ($quote->getAllItems() as $item)
		{
			// Only add the configurable product to the gua variable
			if($item->getProductType() === 'configurable')
			{
				...
				$collection = Mage::getResourceModel('catalog/product_collection')
					->addFieldToFilter('entity_id', array($productId))
					->addAttributeToSelect(array('sku','name','price'))
					->setPageSize(1);

				$product = $collection->getFirstItem();

				$checkoutGua[$product->getSku()] = new Varien_Object(array(
					'id' => $product->getSku(),
					'qty' => $item->getQty(),
					'name' => $product->getName(),
					'price' => $product->getPrice(),
