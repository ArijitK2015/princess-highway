...
/**
 * Class FactoryX_Lookbook_Adminhtml_LookbookController
 */
class FactoryX_Lookbook_Adminhtml_LookbookController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('factoryx_menu/lookbook');
    }
    ...
    {
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('lookbook/adminhtml_lookbook'));
        $this->renderLayout();
    }
        ...
        if ($data = $this->getRequest()->getPost())
        {
            ...
            if ($data['lookbook_type'] == "category")
            {
                if (!$data['category_id']) {
                    ...
                    $error = Mage::helper('lookbook')->__('No category selected');
                    Mage::getSingleton('adminhtml/session')->addError($error);
                    session_write_close(); // render messages
                    $this->_redirect('*/*/choosecat');
                }

                // We add the category_id to the session
                Mage::getSingleton('adminhtml/session')->setCategory(str_replace("category/", "", $data['category_id']));
            }
            else
            {
                // We add the category_id to the session
        ...
        if ($id = $this->getRequest()->getParam('id'))
        {
            ...
            if ($model->getId())
            {
                ...
                if ($model->getLookbookType() == 'category')
                    Mage::getSingleton('adminhtml/session')->setCategoryId($model->getCategoryId());
            }
        }
        ...
        if ($model->getId() || $id == 0)
        {
            ...
            else
            {
                ...
                if (Mage::getSingleton('adminhtml/session')->getChangingType(true))
                {
                    ...
                    // This is the array of attributes to save
                    ...
                    if ($lookbookType != $model->getLookbookType())
                    {
                        ...
                        $saveFlag = true;
                        $attributesToSave[] = 'lookbook_type';
            ...
            $this->getLayout()->getBlock('head')->addItem('js_css','lib/factoryx/lookbook/jquery.ddslick.css');
            $this->getLayout()->getBlock('head')->addItem('skin_css','css/factoryx/lookbook/styles.css');
            $this->getLayout()->getBlock('head')->addJs('lib/factoryx/lookbook/jquery.ddslick.js');
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost())
        {
            ...
            Mage::helper('lookbook')->log(sprintf("%s->lookbook=%s", __METHOD__, print_r($data, true)) );
            ...
            try
            {
                ...
                {
                    $uploader = Mage::getModel('core/file_uploader','shop_pix');
                ...
                // A category must be provided if the include under category nav is set
                if ($data['include_in_nav'] == "category" && (!array_key_exists('nav_category',$data) || !$data['nav_category']))
                {
                    // We use session to set the active tab to show where the error is
                    Mage::getSingleton('admin/session')->setActiveTab('general_tab');
                    throw new Exception ("If you want to display a lookbook link under a category navigation, you must pick a category.");
                }
                ...
                $nextId = $model->getId() ? $model->getId() : Mage::helper('lookbook')->getNextId();

                // Generate the event array
                $eventData = array('id'	=> $nextId, 'lookbook' => $data);

                // Dispatch the event
                Mage::dispatchEvent('lookbook_save_before',$eventData);

                // Save the lookbook
                $model->save();

                // Handle gallery after
                if (isset($data['lookbook_image']) && isset($media_gallery))
                {
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($lookbookIds)) {
            ...
            try {
                ...
                foreach ($lookbookIds as $lookbookId) {
                    Mage::getModel('lookbook/lookbook')
                        ->load($lookbookId)
    ...
    /**
     * @param FactoryX_Lookbook_Model_Lookbook $model
     * @param $value
     * @return array
     */
     ...
     *
     * @return FactoryX_Lookbook_Model_Lookbook_Media
     */
     ...
     *
     * @return FactoryX_Lookbook_Model_Lookbook_Media_Config
     */
