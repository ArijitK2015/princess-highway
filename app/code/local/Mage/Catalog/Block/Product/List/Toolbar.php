<?php
/**
	remove all option from toolbar
	- not sure why, most likely performance - caching should fix this now
*/
...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    public function getAvailableLimit()
    {
        ...
        if (in_array($currentMode, array('list', 'grid'))) {
            $limit = $this->_getAvailableLimit($currentMode);
        }
        else {
            $limit = $this->_defaultAvailableLimit;
        }
        //unset($limit['all']);
        //Mage::log(__METHOD__ . "->limit=" . print_r($limit, true));
        return $limit;
    }
    ...
    protected function _getAvailableLimit($mode)
    {
        ...
        //Mage::log(__METHOD__ . "->perPageValues=" . print_r($perPageValues, false));
        return $perPageValues;
    }
