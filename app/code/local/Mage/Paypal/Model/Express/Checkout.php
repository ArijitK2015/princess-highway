...
/**
!!!!!!!!!!!!
File has been updated by Raph on 21/06/2013 with the 1.7.0.1 version
Except the following lines which are commented due to PayPal Express module being too old for this feature: lines 310 to 312, 316 to 318 and 625 to 627

Due to this problem: https://www.assembla.com/spaces/factoryx/tickets/444?comment=315257973#comment:315257973
The following fix has been removed.
!!!!!!!!!!!!!
import email from paypal into order, so that customer gets order emails
...
# fix previous orders via mysql

-- backup table first
mysqldump shopdang_mage sales_flat_order > sales_flat_order.sql

-- test join and where clause
select customer_email, SUBSTR(additional_information, 137, INSTR(SUBSTR(additional_information, 137),"\"") - 1) as paypal_email from sales_flat_order AS sfo
left join sales_flat_order_payment AS sfop on sfo.entity_id = sfop.parent_id
where customer_group_id = 0 AND method = "paypal_express" AND (customer_email = "" OR customer_email IS NULL)
limit 5;

-- pray - as limits don't work
update sales_flat_order AS sfo
left join sales_flat_order_payment AS sfop on sfo.entity_id = sfop.parent_id
SET sfo.customer_email = SUBSTR(additional_information, 137, INSTR(SUBSTR(additional_information, 137),"\"") - 1)
where customer_group_id = 0 AND method = "paypal_express" AND (customer_email = "" OR customer_email IS NULL);

-- check
select customer_email, SUBSTR(additional_information, 137, INSTR(SUBSTR(additional_information, 137),"\"") - 1) as paypal_email from sales_flat_order AS sfo
left join sales_flat_order_payment AS sfop on sfo.entity_id = sfop.parent_id
where customer_group_id = 0 AND method = "paypal_express"
limit 5;
...
class Mage_Paypal_Model_Express_Checkout
{
    ...
    protected function _prepareCustomerQuote()
    {
		...
		/* Remove old fix (see start of the file)
        // fix paypal express not getting email
