<?php
/**
	Remove the filter and sort by news from date
	Add filter and sort by created at date
	Add updated at as lastUpdate attribute
**/
/**
 * Magento
 *
...
*/
		...
		//$fromDate = strftime("%Y-%m-%d %H:%M:%S", $from);
		$fromDate = $product->getResource()->formatDate(
			mktime(0,0,0,date("n"),date("j"),date("Y"))
		);
		//$toDate = strftime ("%Y-%m-%d %H:%M:%S", $to);
		$toDate = $product->getResource()->formatDate(
			mktime(23,59,59,date("n"),date("j"),date("Y"))
		);
		//Mage::log(__METHOD__ . "=from:" . $fromDate . ",to:" . $toDate);
    ...
    public function addNewItemXmlCallback($args)
    {
        ...
        $data = array(
                ...
                'lastUpdate'    => strtotime($product->getUpdatedAt())
            );
