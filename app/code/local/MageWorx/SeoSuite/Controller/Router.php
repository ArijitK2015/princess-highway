...
class MageWorx_SeoSuite_Controller_Router extends Mage_Core_Controller_Varien_Router_Standard {
    ...
    protected function _setHiddenAttributeToRequest($param) {
		...
		$colorFilter = false;
        $attributeCode = Mage::helper('seosuite')->getColourAttribute();
        foreach ($attr as $attrCode => $attrData) {
            if (isset($attrData['options'][$param[0]])) {
                if (is_array($attributeCode))
                {
                    // Set the chosen color using session
                    if (in_array($attrCode,$attributeCode))
                    {
                        $colorFilter = true;
                        Mage::getModel('core/session')->setFilteredColor($param[0]);
                    }
                }
                else
                {
		...
		if (!$colorFilter) Mage::getModel('core/session')->unsFilteredColor();
    }
