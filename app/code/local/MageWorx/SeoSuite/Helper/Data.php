...
class MageWorx_SeoSuite_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getCanonicalUrl($product) {
        ...
        $canonicalUrl = trim($canonicalUrl,'/');
        if ($canonicalUrl) {            
            $canonicalUrl = $this->_trailingSlash(Mage::getUrl('') .$canonicalUrl);
        }
    ...
    {
        // If there is a colour configured
        if ($attr = Mage::getStoreConfig('mageworx_seo/seosuite/colour_attribute'))
        {
            return $attr;
        }
        else
        {
            // Return an array with every possible colour attributes
            return array('colour_all','colour','colour_description');
        }
    }
}
